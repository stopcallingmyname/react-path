import './Path.scss'

export default function Path() {
    return (<>
        <div className='container'>
			<div className='content-block'>
				<div className='date-container'>
					<h1>08<span className='colored'>.</span>
						05<span className='colored'>.</span>
						2023
					</h1>
					
					<svg className='pin-icon'></svg>
				</div>
				<h2>малень<span className='colored'>к</span>ое начал<span className='colored'>о</span> большой ис<span className='colored'>т</span>ории</h2>
			</div>
				<div className='path-container'>
                    <div className='path-block'>
                    <div className='path-date --r'>
						<div className='step'>
						<svg className='heart-icon'></svg>
						<h3>08<span className='colored'>.</span>
							06<span className='colored'>.</span>
							2023</h3>
						</div>
					</div>
					<div className='path-date --l'>
						<div className='step'>
						<svg className='heart-icon'></svg>
						<h3>08<span className='colored'>.</span>
							07<span className='colored'>.</span>
							2023</h3>
						</div>
					</div>
					<div className='path-date --r'>
						<div className='step'>
						<svg className='heart-icon'></svg>
						<h3>08<span className='colored'>.</span>
							08<span className='colored'>.</span>
							2023</h3>
						</div>
					</div>
					<div className='path-date --l'>
						<div className='step'>
						<svg className='heart-icon'></svg>
						<h3>08<span className='colored'>.</span>
							09<span className='colored'>.</span>
							2023</h3>
						</div>
					</div>
					<div className='path-date --r'>
						<div className='step'>
						<svg className='heart-icon'></svg>
						<h3>08<span className='colored'>.</span>
							10<span className='colored'>.</span>
							2023</h3>
						</div>
					</div>
					<div className='path-date --l'>
						<div className='step'>
						<svg className='heart-icon'></svg>
						<h3>08<span className='colored'>.</span>
							11<span className='colored'>.</span>
							2023</h3>
						</div>
					</div>
					<div className='path-date --r'>
						<div className='step'>
						<svg className='heart-icon'></svg>
						<h3>08<span className='colored'>.</span>
							12<span className='colored'>.</span>
							2023</h3>
						</div>
					</div>
					<div className='path-date --l'>
						<div className='step'>
						<svg className='heart-icon'></svg>
						<h3>08<span className='colored'>.</span>
							01<span className='colored'>.</span>
							2024</h3>
						</div>
					</div>
					<div className='path-date --r' >
						<div className='step'>
						<svg className='heart-icon'></svg>
						<h3>08<span className='colored'>.</span>
							02<span className='colored'>.</span>
							2024</h3>
						</div>
                        <div className='notification-heart-icon'></div>
					</div>
                    </div>
					
                    <div className='message'>
                        <div className='separator'></div>
                        <h4>время пролетело, как мне кажется, мгновенно.</h4>
                        <h4>За э<span className='colored'>т</span>и 9 месяцев м<span className='colored'>ы</span> вместе пережили огро<span className='colored'>м</span>ное количеств<span className='colored'>о</span> эмоций, кучу неверо<span className='colored'>я</span>тных историй и путешествий, хоть и небольших, но не менее захватывающих!</h4>
                        <h4>встретили вместе множество закатов и рассветов, побывали во многих живописных местах, ох, а сколько всего нам еще предстоит в<span className='colored'>м</span>есте пройти!</h4>
                        <h4>безумно благодар<span className='colored'>е</span>н тебе за все, <span className='colored'>ч</span>то ты делаешь для нас и для меня в час<span className='colored'>т</span>ности.</h4>
                        <h4>Я благод<span className='colored'>а</span>рен тебе за каждый миг, проведенный вместе, рядом с тобой мир действительно оживает.</h4>
                    </div>
                    <div className='couple-icon'></div>
                    <div className='ending'>
                        <h2>безумно тебя люблю</h2>
                        <svg className='heart-icon'></svg>
                    </div>
			</div>
			
		</div>
      {/* <div className="container">
			<div className="letter">
				<h1>Your smile is as beautiful as rose, I hope you can always smile</h1>
			</div>
			<div className="rose">
				<div className="flower">
					<div className="petal"></div>
					<div className="petal"></div>
					<div className="petal"></div>
					<div className="petal"></div>
					<div className="petal"></div>
				</div>
				<div className="leaf">
					<div className="stem"></div>
					<div className="leafs"></div>
					<div className="leafs"></div>
				</div>
			</div>

		</div> */}
    {/* <Cake /> */}
    </>)
}